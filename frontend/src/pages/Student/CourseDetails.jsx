...
const CourseHeader = ({ course, handleEnrollStudent, isEnrolled = false }) => {

  if (!course) return <Box>Loading course details...</Box>;
  const redirect = () => {
    window.location.href = "http://localhost:5174/";
  };

  return (
    <Card shadow="lg" variant="outline" borderColor="indigo.100" overflow="hidden">
      {/* Course Image Banner */}
      {course?.image?.url && (
        <Box position="relative">
          ...
          <Box
            ...
            p={4}
          >
            <Heading size="xl" color="white">{course.name}</Heading>
          </Box>
        </Box>
      )}
      ...
      <CardBody p={6}>
        <Stack direction={{ base: "column", md: "row" }} spacing={8}>
          {/* Main Course Info */}
          <VStack flex="3" align="stretch" spacing={5}>
            {!course?.image?.url && (
              <Heading size="xl" color="indigo.700">{course.name}</Heading>
            )}

            <Text fontSize="md" color="gray.700">{course.description}</Text>
            ...
            {course.instructor && (
              <HStack mt={3}>
                ...
                />
                <Box>
                  <Text fontWeight="bold">Instructor: {course.instructor.name}</Text>
                  <Text fontSize="sm" color="gray.600">{course.instructor.branch}</Text>
                </Box>
              </HStack>
            )}
            ...
            {course._id === "67ddd57dbb38704921a5a142" && (
              <Button
                bg="indigo.600"
                color="indigo.700"
                variant="solid"
...
const AssignmentCard = ({ item, index, user, handleFileChange, handleFileUpload, hasSubmitted, file, grades, handleGrade, isOpen, onOpen, onClose }) => {
  return (
    ...
    >
      <CardHeader bg="indigo.50" borderBottomWidth="1px" borderColor="indigo.100">
        <Heading size="md" color="indigo.600">Assignments</Heading>
      </CardHeader>
        ...
        {user.role === "Student" && (
          <VStack spacing={4} align="stretch">
            ...
            >
              Upload Assignment
              {/* Additional Text */}
              <span style={{ fontSize: "0.9rem", marginLeft: "8px", color: "#000" }}>
                (Only PDF Allowed)
              </span>
              <Input
                id={`file-input-${index}`}
    ...
    <Card>
      ...
      <Box p={6} borderWidth="1px" borderRadius="lg" mb={6} mt={8} bg="white" shadow="md">
        ...
        <HStack spacing={4}>
          <Button
            bg="indigo.600"
            color="gray.800" // Set to a darker color
...
const fetchMetadata = async (url) => {
  try {
    const response = await fetch(`https://api.linkpreview.net/?key=fc490684baf766d5b91a39952561115a&q=${encodeURIComponent(url)}`);
    const data = await response.json();
    return data.image || ""; // Returns the fetched preview image or empty string if not found
  } catch (error) {
...
const RelatedCourses = ({ relatedCourses, isLoadingRelated }) => {
  ...
  useEffect(() => {
    relatedCourses.forEach(async (course) => {
      if (!course.thumbnail && course.url) {
        const imageUrl = await fetchMetadata(course.url);
        setThumbnails((prev) => ({ ...prev, [course.url]: imageUrl }));
      }
    });
  }, [relatedCourses]);
          ...
          {isLoadingRelated ? (
            ...
            <Flex gap={4} flexWrap="wrap" justifyContent="flex-start">
              {relatedCourses?.map((course, index) => (
                ...
                >
                  <Image
                    src={course?.thumbnail || thumbnails[course?.url] || `https://source.unsplash.com/300x200/?${encodeURIComponent(course?.title || 'course')}`}
                    alt={course?.title || "Course"}
                    objectFit="cover"
                    height="150px"
                  />
                  <CardBody>
                    {/* Course title is now clickable and links to the course URL */}
                    <Heading size="sm" mb={2}>
                      <Link href={course?.url || "#"} isExternal color="indigo.600" _hover={{ textDecoration: "underline", color: "indigo.800" }}>
                        {course?.title || "Untitled Course"}
                      </Link>
                    </Heading>
                    <Text fontSize="sm" color="gray.600" mb={3}>{course?.platform || "Unknown Platform"}</Text>
                    <Button
                      as={Link}
                      href={course?.url || "#"}
                      isExternal
...
export default function CourseDetail() {
  ...
  useEffect(() => {
    ...
    const fetchRelatedCourses = async () => {
      ...
      try {
        const response = await axios.get(`${getSimilarCourses}/${id}`, {
          withCredentials: true,
        });

        if (response.data.success) {
          setRelatedCourses(response.data.similarCourses);
        }
      } catch (error) {
      ...
      <Container maxW="container.xl" py={8}>
        <Card shadow="md" variant="outline" borderColor="indigo.100">
          ...
          <CardBody>
            <VStack spacing={4} align="stretch">
              <Text>
                This course is not currently selected or you haven't enrolled
                yet.
              </Text>
              <Text>
                Enroll in this course to access all materials and assignments.
              </Text>
              <Button
                colorScheme="indigo"
                size="lg"
  ...
  return (
    <Container maxW="container.xl" py={8}>
      <VStack spacing={8} align="stretch">
        ...
        {user.role === "Student" && (
          <PersonalizedRoadmap courseId={id} studentId={user._id} />
        )}
